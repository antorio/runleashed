...
# unleashed.globals.expression_clamp; 0 (default) disables clamping so the target
...
def _clamp_mag():
    ...
    try:
        return float(getattr(unleashed.globals, 'expression_clamp', 0.0) or 0.0)
    except Exception:
        return 0.0
...
# when unleashed.globals.expression_keep_structure is on.
...
def build_applied_expression(temp_exp, target_exp, factor, use_eyes, use_mouth, use_brows):
    ...
    # `power` (unleashed.globals.expression_power) lets you over-drive subtle
    # expressions so they become clearly visible; 1.0 = natural target amount.
    import unleashed.globals
    power = float(getattr(unleashed.globals, 'expression_power', 1.0) or 1.0)
    ...
    if not use_brows:
        reset(EXP_IDX_BROWS)
    if getattr(unleashed.globals, 'expression_keep_structure', False):
