...
- information only: head angles, mouth open, and whether face shape uses it
  (unleashed/face_shape.pick_shape_photos, the same choice the swap makes).
...
def photo_metrics(face, image, geometry=True):
    ...
    if geometry:
        try:
            from unleashed.face_shape import photo_geometry, source_landmarks
...
def check(metrics):
    ...
    from unleashed.face_shape import pick_shape_photos
    n = len(metrics)
