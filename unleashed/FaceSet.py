...
class FaceSet:
    ...
    def __init__(self):
        ...
        self.average_params = None      # (mode, threshold) the average was made with
    ...
    def AverageEmbeddings(self):
        ...
        mode = getattr(unleashed.globals, 'faceset_average_mode', 'robust')
        thr = float(getattr(unleashed.globals, 'faceset_outlier_threshold', 0.6))
        self.average_params = (mode, thr)
        try:
            ...
            # A threshold below the closest photo keeps just that photo (the
            # 'median' pick): it used to fall back to keeping ALL photos, so the
            # strictest settings dropped nothing at all.
            dist = 1.0 - (unit @ center_u)          # cosine distance to median
            keep = dist <= max(thr, float(dist.min()))
    ...
    def reaverage(self):
        """The average again with the current Settings (mode, threshold): the
        original identity of faces[0] is put back first."""
        if len(self.faces) <= 1 or self.embeddings_backup is None:
            return
        self.faces[0]['embedding'] = self.embeddings_backup
        self.embeddings_backup = None
        self.AverageEmbeddings()

    def average_is_current(self):
        mode = getattr(unleashed.globals, 'faceset_average_mode', 'robust')
        thr = float(getattr(unleashed.globals, 'faceset_outlier_threshold', 0.6))
        return self.average_params is None or self.average_params == (mode, thr)
