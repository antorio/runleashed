...
class ProcessMgr():
    # FIX: moved all mutable state to instance variables (was class variables — shared across instances)
    def __init__(self, progress):
        ...
        # FIX: temporal smoothing — EMA of embeddings per target face index
        self._embedding_ema: dict = {}
        self._EMA_ALPHA = 0.35  # lower = smoother but slower to adapt
    ...
    def initialize(self, input_faces, target_faces, options):
        ...
        self.num_frames_no_face = 0
        self.last_swapped_frame = None
        self._embedding_ema = {}  # reset EMA on new session
        ...
        if self.options.swap_mode == "first":
            ...
            if self.options.swap_mode == "all":
                ...
                for i, tf in enumerate(self.target_face_datas):
                    # FIX: use EMA-smoothed embedding for matching to reduce flicker
                    ref_embedding = self._get_smoothed_target_embedding(i, tf.embedding)
                    for face in faces:
                        if compute_cosine_distance(ref_embedding, face.embedding) <= self.options.face_distance_threshold:
                            if i < len(self.input_face_datas):
                                if use_index:
                                    temp_frame = self.process_face(self.options.selected_index, face, temp_frame)
                                else:
    ...
    # NEW: EMA smoothing of target embedding for stable matching across frames
    def _get_smoothed_target_embedding(self, face_index: int, raw_embedding: np.ndarray) -> np.ndarray:
        if face_index not in self._embedding_ema:
            self._embedding_ema[face_index] = raw_embedding.copy()
        else:
            alpha = self._EMA_ALPHA
            self._embedding_ema[face_index] = (
                alpha * raw_embedding + (1.0 - alpha) * self._embedding_ema[face_index]
            )
            # keep normalized
            norm = np.linalg.norm(self._embedding_ema[face_index])
            if norm > 0:
                self._embedding_ema[face_index] /= norm
        return self._embedding_ema[face_index]
    ...
    # NEW: determine model_type from modelname so normalization is correct per model
    def _get_model_type(self) -> str:
        if self.options is None:
            return 'inswapper'
        name = getattr(self.options, 'swap_modelname', '').lower()
        if 'reswapper' in name:
            return 'reswapper'
        return 'inswapper'
    ...
    def process_face(self, face_index, target_face: Face, frame: Frame):
        ...
        for p in self.processors:
            if p.type == 'swap':
                ...
                fake_frame = self.apply_unsharp_mask(fake_frame, strength=0.45)
                scale_factor = 0.0
            elif p.type == 'mask':
    ...
    # NEW: unsharp mask to restore fine detail after pixel-boost compositing
    def apply_unsharp_mask(self, img: np.ndarray, strength: float = 0.5, radius: int = 3) -> np.ndarray:
    ...
    def paste_upscale(self, fake_face, upsk_face, M, target_img, scale_factor, mask_offsets):
        ...
        # NEW: histogram matching to reduce color/brightness mismatch at borders
        paste_face_uint8 = np.clip(paste_face, 0, 255).astype(np.uint8)
        mask_bool = (img_matte[:, :, 0] > 0.05)
        paste_face_uint8 = self.histogram_match_face(paste_face_uint8, target_img, mask_bool)
        paste_face = paste_face_uint8.astype(np.float32)
    ...
    # NEW: histogram matching in Lab colorspace — skin-tone aware, preserves luminance structure
    def histogram_match_face(self, swapped: np.ndarray, target: np.ndarray, face_mask: np.ndarray) -> np.ndarray:
        """
        Match swapped face color distribution to the surrounding skin in the target image.
        Uses only skin-like pixels from a border ring around the face mask as reference,
        so hair/clothing don't pollute the color transfer.
        """
        try:
            # Build a reference region: pixels just outside the face mask (ring)
            kernel = np.ones((21, 21), np.uint8)
            dilated = cv2.dilate(face_mask.astype(np.uint8), kernel, iterations=1)
            border_ring = (dilated > 0) & (~face_mask)

            # Filter border ring to skin-like pixels using YCrCb heuristic
            ycrcb = cv2.cvtColor(target, cv2.COLOR_BGR2YCrCb)
            skin_mask = (
                (ycrcb[:, :, 1] >= 133) & (ycrcb[:, :, 1] <= 177) &
                (ycrcb[:, :, 2] >= 77)  & (ycrcb[:, :, 2] <= 127)
            )
            ref_mask = border_ring & skin_mask

            # Fallback to full border ring if too few skin pixels
            if ref_mask.sum() < 100:
                ref_mask = border_ring
            if ref_mask.sum() < 50:
                return swapped  # not enough reference — skip
            ...
            # Per-channel: match mean+std of swapped face to reference ring
            for c in range(3):
                src_vals = swapped_lab[:, :, c][face_mask]
                ref_vals = target_lab[:, :, c][ref_mask]

                if src_vals.std() < 1e-3 or ref_vals.std() < 1e-3:
                    continue

                # Mild scaling: interpolate 50% toward full match to avoid overcorrection
                adjusted = (src_vals - src_vals.mean()) * (ref_vals.std() / src_vals.std()) + ref_vals.mean()
                blended  = src_vals * 0.5 + adjusted * 0.5
                result_lab[:, :, c][face_mask] = blended

            result = cv2.cvtColor(np.clip(result_lab, 0, 255).astype(np.uint8), cv2.COLOR_Lab2BGR)
            return result

        except Exception as e:
            print(f"histogram_match_face error: {e}")
            return swapped
    ...
    # FIX: model_type now driven by actual model name, not hardcoded
    ...
    # FIX: model_type now driven by actual model name
    ...
    def create_mouth_mask(self, face: Face, frame: Frame):
        mouth_cutout = None
        # FIX: initialize coords so return never references unbound variables
    ...
    def apply_mouth_area(self, frame: np.ndarray, mouth_cutout: np.ndarray, mouth_box: tuple) -> np.ndarray:
        ...
        try:
            ...
            # NEW: use skin-aware color transfer for mouth area
    ...
    def apply_color_transfer(self, source, target):
        ...
        FIX: added guard against zero std to prevent NaN when src/target are flat color.
        """
        ...
        # FIX: avoid division by zero when source channel is flat
        source_std = np.where(source_std < 1e-6, 1e-6, source_std)
