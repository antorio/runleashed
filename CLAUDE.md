...
## Tujuan kualitas: kemiripan source (dibahas 24 Sep)
- Source user: faceset .fsz; ER biasanya ON; mask DFL XSeg atau FF XSeg 3; swap steps jarang dipakai karena wajah terlihat makin mengecil tiap step.
- Keluhan: dagu/bagian bawah wajah target tetap terlihat, bahkan polos (tanpa ER/XSeg) dan frontal; dagu target yang kuat (mis. belah) mengalahkan source. Penyebab cakupan sudah ditemukan (toggle di atas). Yang tersisa setelah itu = inswapper sendiri mempertahankan struktur target (batas model).
- Kandidat berikutnya (belum dicoba): dorong identitas menjauhi target (embedding; FaceFusion punya kenop serupa tapi untuk inswapper mencampur ruang vektor berbeda), panjang vektor identitas (insightface menormalkan, FaceFusion tidak), set keypoint ER, DMDNet (referensi foto source, pernah ada di upstream), transfer bentuk wajah (eksperimental). Usulan: "lab kemiripan" di Colab (grid visual + skor dengan pengenal wajah yang berbeda dari inswapper).
...
4. **Erosi matte bergantung rotasi wajah** (diukur): `blur_area` mengerosi di ruang frame dengan kernel kotak lurus, dan ukuran kernelnya dari kotak pembatas → wajah miring 25° tererosi ±75% lebih dalam di semua sisi (dagu panjang 55% ter-swap vs 99% frontal pada setting default). Relevan untuk keluhan wajah miring. Toggle dagu hanya memperbaiki sisi bawah.
