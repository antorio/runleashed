...
- **Face Management diperbaiki** (dari kode; alur diuji dengan Gradio tiruan): satu daftar `entries` (dulu thumbs/images terpisah → Remove salah indeks, Create menggandakan isi karena event change-nya memuat ulang .fsz). Input yang sama tidak ditambah dua kali (disinkronkan dengan isi Input Files). Membuka .fsz MENGGANTI daftar (satu wajah terbesar per PNG). Create menulis `faceset_<tanggal_jam>.fsz` baru (tidak menimpa). Remove butuh pilihan; pilihan mengikuti perilaku Gallery. Semua event Face Management berurutan (`concurrency_id='facemgr'`).
